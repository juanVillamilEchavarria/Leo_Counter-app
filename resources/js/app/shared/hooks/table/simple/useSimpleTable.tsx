...
export default function useSimpleTable<TData extends Record<string, any>>({
  data = [],
  pageSize = 10,
  tableColumns,
  formModelHook
  ...
  data: TData[],
  pageSize?: number,
  tableColumns: ({onDelete}: {onDelete: (item: TData) => void}) => SimpleTableColumn<TData>[],
  formModelHook?: any
}) {
  const [itemSelected, setItemSelected] = useState<TData | null>(null)
  const columns = useMemo(() => 
  tableColumns({ onDelete: setItemSelected }),
  [tableColumns]
)
